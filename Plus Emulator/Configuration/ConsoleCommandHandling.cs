                    ...
                    case "wualert":
                        {
                            ...
                            string Notice = ChatCommandHandler.MergeParams(parameters, 2);
                    ...
                    case "ban":
                        {
                            ...
                            if (dced)
                            {
                                ...
                                {
                                    Plus.GetGame().GetBanManager().BanUser(client, "Console", HabboHotel.Support.ModerationBanType.IP, name.ToLower(), "Automatic Ban via Console [Urgent]", 9999999.0);
                                    //.dbClient.dbClient.RunFastQuery
                              }
                            }
                            else
                            {
                                ...
                                using (IQueryAdapter dbClient = Plus.GetDatabaseManager().GetQueryReactor())
                                {
                                    ...
                                    dbClient.SetQuery("INSERT INTO `users_bans` (`bantype`, `value`, `reason`) VALUES ('user', '" + name.ToLower() + "', Automatic Ban via Console [Urgent]");
                                }
                    ...
                    /*    case "fire":
                        {
                            ...
                            if (dced)
                            {
                               User.GetConnection().Dispose();
                                ...
                                Console.WriteLine("Successfully fired " + name.ToLower(), ConsoleColor.Red);
                                ...
                                using (IQueryAdapter dbClient = Plus.GetDatabaseManager().GetQueryReactor())
                                {
                                    ...
                                    client.GetRoleplay().SaveJobComponents();
                                    Plus.GetGame().GetModerationTool().LogStaffEntry("Console", client.GetHabbo().UserName, "Job Fire", "[URGENT] Fire From Console");
                            ...
                            else
                            {
                                ...
                                using (IQueryAdapter dbClient = Plus.GetDatabaseManager().GetQueryReactor())
                                {
                                    dbClient.RunFastQuery("UPDATE rp_stats SET job_rank = '1' WHERE id = '" +client.GetHabbo().Id + "'");
                                    dbClient.RunFastQuery("UPDATE rp_stats SET job_id = '1' WHERE id = '" + client.GetHabbo().Id + "'");
                                    Plus.GetGame().GetModerationTool().LogStaffEntry("Console", client.GetHabbo().UserName, "Job Fire", "[URGENT] Fire From Console");
                                }
                                                    ...
                                                    case "rpstats":
                                                        {
                                                            ...
                                                            Console.WriteLine("All Statistics saved!");

                                                            break;
                                                        }
                                                    case "jobs":
                                                        {
                                                            Console.WriteLine("Updating Jobs..");
                                                            JobManager.init();
                                                            Console.WriteLine("All Jobs updated!");
