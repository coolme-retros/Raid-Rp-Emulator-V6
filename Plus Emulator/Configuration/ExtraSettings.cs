...
namespace Plus.Configuration
{
    ...
    internal class ExtraSettings
    {
        ...
        /// The ServerCamera from Stories
        /// </summary>
        internal static string ApiHotelDomain = "";
        ...
        {
            if (File.Exists("Settings/Welcome/message.txt")) WelcomeMessage = File.ReadAllText("Settings/Welcome/message.txt");
            if (!File.Exists("Settings/other.ini")) return false;
            ...
            foreach (var @params in from line in File.ReadAllLines("Settings/other.ini", Encoding.Default) where !string.IsNullOrWhiteSpace(line) && line.Contains("=") select line.Split('='))
            {
                switch (@params[0])
                {
                    ...
                    case "currency.loop.time.in.minutes":
                        int i;
                        if (int.TryParse(@params[1], out i)) CURRENTY_LOOP_TIME_IN_MINUTES = i;
                        break;

                    case "credits.to.give":
                        int j;
                        if (int.TryParse(@params[1], out j)) CREDITS_TO_GIVE = j;
                        break;

                    case "pixels.to.give":
                        int k;
                        if (int.TryParse(@params[1], out k)) PIXELS_TO_GIVE = k;
                        break;
                    ...
                    case "diamonds.to.give":
                        int l;
                        if (int.TryParse(@params[1], out l)) DIAMONDS_TO_GIVE = l;
                        break;
                    ...
                    case "newuser.gift.yttv2.id":
                        uint u;
                        if (uint.TryParse(@params[1], out u)) NEW_USER_GIFT_YTTV2_ID = u;
                        break;
                    ...
                    case "api.hotel.domain":
                        ApiHotelDomain = @params[1];
                        break;
