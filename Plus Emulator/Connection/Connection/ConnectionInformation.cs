...
namespace Plus.Connection.Connection
{
    public class ConnectionInformation : IDisposable
    {
        ...
        public void startPacketProcessing()
        {
            ...
            {
                isConnected = true;
                Out.WriteLine("Starting packet processsing of client [" + this.connectionID + "]");
                try
                {
        ...
        public void disconnect()
        {
            try
            {
                if (isConnected)
                {
                    ...
                    Out.WriteLine("Connection [" + this.connectionID + "] has been disconnected");
                    try
                    {
        ...
        {
            Out.WriteLine("[DEBUG] Packet received from client [" + this.connectionID + "]");
            int bytesReceived;
            try
            {
            ...
            if (bytesReceived == 0)
            {
                Console.WriteLine("No bytes received from incomming Packet Data");
                disconnect();
                return;
            }
