    ...
    internal static class CatalogPacket
    {
        ...
        internal static ServerMessage ComposeIndex(uint rank, string type)
        {
            IEnumerable<CatalogPage> pages =
                Plus.GetGame().GetCatalog().Categories.Values.OfType<CatalogPage>().ToList();
            ...
            foreach (var cat in sortedPages)
            {
                ...
                var sortedSubPages =
                    pages.Where(x => x.ParentId == cat.PageId && x.MinRank <= rank).OrderBy(x => x.Caption);
...

            {
                ...
                foreach (var baseItem in item.Items.Keys)
                {
                    ...
                    if (item.Name != "room_ad_plus_badge")
                    {
                        ...
                        else if (item.Name.StartsWith("bot_") ||
                                 baseItem.InteractionType == Interaction.MusicDisc || item.GetFirstBaseItem().Name == "poster")
                        ...
                        else if (item.SongId > 0u &&
                                 baseItem.InteractionType == Interaction.MusicDisc) message.AppendString(item.ExtraData);
                        else message.AppendString(string.Empty);
