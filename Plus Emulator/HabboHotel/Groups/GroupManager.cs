...
namespace Plus.HabboHotel.Groups
{
    ...
    class GroupManager
    {
        ...
        internal Guild GetGroup(uint groupId)
        {
            ...
            using (var queryReactor = Plus.GetDatabaseManager().GetQueryReactor())
            {
                queryReactor.SetQuery("SELECT * FROM rp_jobs WHERE id=" + groupId + " LIMIT 1");
                var row = queryReactor.GetRow();
                if (row == null)
                    return null;
                queryReactor.SetQuery("SELECT id,job_id,job_rank FROM rp_stats WHERE job_id=" + groupId);
                var table = queryReactor.GetTable();
                ...
                {
                    dictionary.Add(Convert.ToUInt32(dataRow[0]), new GroupUser(Convert.ToUInt32(dataRow[0]), Convert.ToUInt32(groupId), Convert.ToInt32(dataRow[2])));
                    if (Convert.ToInt16(dataRow[1]) == HabboHotel.Roleplay.Jobs.JobManager.JobData[(int)groupId].OwnerId)
                    {
                        dictionary2.Add(Convert.ToUInt32(dataRow[0]), new GroupUser(Convert.ToUInt32(dataRow[0]), Convert.ToUInt32(groupId), Convert.ToInt32(dataRow[1])));
                    }
                }
                ...
                var group = new Guild(Convert.ToUInt32(row[0]), row[1].ToString(), row[2].ToString(), Convert.ToUInt32(row[6]), row[3].ToString(), Convert.ToInt32(row[5]), Convert.ToUInt32(row[4]), 0, 0, dictionary, list, dictionary2, 0, 0, row["has_forum"].ToString() == "1", row["forum_name"].ToString(), row["forum_description"].ToString(), uint.Parse(row["forum_messages_count"].ToString()), double.Parse(row["forum_score"].ToString()), uint.Parse(row["forum_lastposter_id"].ToString()), row["forum_lastposter_name"].ToString(), int.Parse(row["forum_lastposter_timestamp"].ToString()));
                if (!Groups.Contains(Convert.ToUInt32(row[0])))
                {
                    Groups.Add(Convert.ToUInt32(row[0]), group);
                }
                return group;
            }
        }
        ...
        internal HashSet<GroupUser> GetUserGroups(uint userId)
        {
            ...
            using (var queryReactor = Plus.GetDatabaseManager().GetQueryReactor())
            {
                queryReactor.SetQuery("SELECT job_id,job_rank FROM rp_stats WHERE id=" + userId);
                var table = queryReactor.GetTable();
                foreach (DataRow dataRow in table.Rows)
                {
                    list.Add(new GroupUser(userId, Convert.ToUInt32(dataRow[0]), Convert.ToInt32(dataRow[1])));
                }
            }
            return list;
        }
