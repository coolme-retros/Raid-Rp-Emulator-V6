    ...
    public class taxiTimer
    {
        ...
        public void timerDone(object info)
        {
            try
            {
                ...
                {
                    Session.SendWhisper("The police stunned you therefore your taxi has been canceled!");
                    stopTimer();
                    return;
                }
              ...
              //  TotalPrice = Price - IncomeTax;
                ...
                if (Session.GetRoleplay().Driving && !Session.GetRoleplay().sdriving)
                {
                    Session.Shout("*As they make it to their destination, they park their car outside of the facility*");
                }

                else if (!Session.GetRoleplay().Driving && !Session.GetRoleplay().sdriving)

                {
                    Session.Shout("*Hops inside their taxi as they see it pull up*");
