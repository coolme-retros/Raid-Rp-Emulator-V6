    ...
    public class Room
    {
        ...
        internal void ProcessRoom()
        {
            try
            {
                ...
                try
                {
                    ...
                    if (!_mCycleEnded)
                    {
                        if ((_idleTime >= 600 && !JustLoaded) || (_idleTime >= 600 && JustLoaded))
                        {
                            Plus.GetGame().GetRoomManager().UnloadRoom(this, "No users");
                            return;
                        }
                        //Unloads Court Room, keeps bot from glitching when there is no trial
                        if ((_idleTime >= 10 && RoleplayManager.CourtTrialIsStarting == false && RoomData.Court))
                        {
                            Plus.GetGame().GetRoomManager().UnloadRoom(this, "No court trial started.");
                        }
        ...
        private void OnRoomCrash(Exception e)
        {
            ...
            foreach (var user in users.Where(user => user != null && user.GetClient() != null))
               ...
               // dbClient.SetQuery("INSERT INTO `rp_room_crashes` (`userid`, `Error Code`, `RoomId`, `Affected User`) VALUES ('','" + EC + "', " + Session.GetHabbo().CurrentRoomId + "' );");
               // dbClient.RunQuery();
            }
