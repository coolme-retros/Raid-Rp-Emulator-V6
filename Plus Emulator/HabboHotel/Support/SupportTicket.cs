...
namespace Plus.HabboHotel.Support
{
    ...
    internal class SupportTicket
    {
            ...
            string roomName, double timestamp, List<string> reportedChats)
        {
            ...
            this._senderName = Plus.GetGame().GetClientManager().GetNameById(senderId);
            this._reportedName = Plus.GetGame().GetClientManager().GetNameById(reportedId);
            this._modName = Plus.GetGame().GetClientManager().GetNameById(this.ModeratorId);
