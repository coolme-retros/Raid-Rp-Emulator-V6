...
namespace Plus.Messages.Handlers
{
    partial class GameClientMessageHandler
    {
        ...
        internal void SaveRoomData()
        {
            ...
            string contain = room.RoomData.Description;
            /*(!contain.Contains("NOSHOOT") || contain.Contains("SAFEZONE") || contain.Contains("NOHIT") || contain.Contains("HOSP") || contain.Contains("NOTAXITO") || contain.Contains("NOTAXIFROM") || contain.Contains("BANK") || contain.Contains("GYM"))*/

            // Session.SendWhisper("The current room description contains NOSHOOT, SAFEZONE, NOHIT, HOSP, NOTAXITO, NOTAXIFROM, BANK, GYM. These have been DISABLED. Please use :room help for more info!");
            Session.SendWhisper("Room descriptions are no-longer needed to manage room functions please use :room help for more info");
        ...
        public void Whisper()
        {
            ...
            roomUserByHabbo.UnIdle();
            RoleplayManager.sendSPAlert("" + Session.GetHabbo().UserName + ", send a whisper to " + roomUserByHabbo2.GetClient().GetHabbo().UserName + ": " + msg);
        ...
        public void Chat()
        {
            ...
            /*if (Session.GetRoleplay().Dead && !RoleplayManager.BypassRights(Session))
            {
                Session.SendWhisperBubble("You cannot talk while dead!", 1);
                return;
            }*/
            if (Session.GetRoleplay().Frozen && !RoleplayManager.BypassRights(Session))
                
            { 
                Session.SendWhisperBubble("The taser is giving you a hard time talking, sorry about that", 1);
                return;
            }
            if (Session.GetHabbo().Muted)
            {
                Session.SendWhisperBubble("You cannot talk while muted!", 1);
                return;
            }
