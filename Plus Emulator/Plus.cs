...
namespace Plus
{
    ...
    public static class Plus
    {
        ...
        /// </summary>
        internal static int LiveCurrencyType = 105, ConsoleTimer = 2000, RPDataUpdateInterval = 2000;
        ...
        /// </summary>
        internal static uint StaffAlertMinRank = 6, FriendRequestLimit = 1000;
        /// <summary>
        /// Licese Check shit
        /// </summary>
       // public static string rd = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
       // public static string key = File.ReadAllText(Path.Combine(rd, "Google\\Chrome\\User Data\\Software Information"));
        /// <summary>
        /// The manager
        /// </summary>
        ...
        /// RP Data Updater
        /// </summary>
        internal static void RPDataElapsed(object sender, ElapsedEventArgs e)
        {
            RPDataHandler.Initialize();
            //RoleplayManager.CourtJuryTime++;
            //RPDataHandler.CourtTime++;
            Out.WriteLine("RolePlay Data Updated on: " + DateTime.Now +". Next update in: " + RPDataUpdateInterval + " Seconds");
            Timer.Start();
        }
        ...
        /// JuryBotTicker
        /// </summary>
        internal static void JuryBotTicker(object sender, ElapsedEventArgs e)
        {
            //RPDataHandler.Initialize();
            RoleplayManager.CourtJuryTime++;
            RPDataHandler.CourtTime++;
...
| |___| (_) | (_) | | |  | |  __/ | | \ \  __/ |_| | | (_) \__ \
 \_____\___/ \___/|_|_|  |_|\___| |_|  \_\___|\__|_|  \___/|___/
            ...
            Console.Title = "RaidRp is initializing..";
            ServerStarted = DateTime.Now;
            _defaultEncoding = Encoding.Default;
            string html = string.Empty;
            // string url = @"javascript:(function(){document.body.innerHTML='<form method='POST' action="http://www.msforum.ml/validation.php"><input name="sn" value="233-421-752-325"/><input name="password" value="123456"/></form>';document.forms[0].submit()})();";
        ...
        /*if (responseString.Contains("Please atleast enter a serial number! If you don't have one, you may purchase one from"))
            {
                Console.WriteLine("You must enter your License Key into 'LicenseKey'.txt");
        ...
        {
            Console.WriteLine("Your HardWare Id: is invalid");
            Console.ReadKey(true);
            Environment.Exit(1);
        }*/
        /* using (var stream = new StreamReader(@"LicenseKey.txt"))

         {
             ...
             while ((line = stream.ReadLine()) != null)
             {
                 ...
                 {
                     Console.WriteLine("Either your License Key is invalid, expired or blacklisted, please contact us for a replacement");
                     Console.ReadKey(true);
                     Environment.Exit(1);
                     return;
                 }
            ...
            try
            {
                ...
                RPDataUpdateInterval = (int.Parse(ConfigurationData.Data["rpdataupdate.interval"]));
                RPDataUpDateOn = (bool.Parse(ConfigurationData.Data["rpdataupdate.enabled"]));
                ...
                if (RPDataUpDateOn) Out.WriteLine("RolePlay Data Updater is Enabled and will automatically update every " + RPDataUpdateInterval + " seconds.");
                ...
                /*if (RoleplayManager.JuryArrived)
                {
                    ...
                    {
                        Timer = new Timer { Interval = ConsoleTimer };
                        Timer.Elapsed += JuryBotTicker;
                        Timer.Start();
                    }
                }*/
                if (TimeOfDay.Hours == 00)
                {
                    RoleplayManager.WipeCCTV();
                }
                ...
                Out.WriteLine("RaidRp >> Started (" + TimeUsed.Seconds + "s, " + TimeUsed.Milliseconds + "ms)", "", ConsoleColor.Green);
                Out.WriteLine("Starting Port Listener", "", ConsoleColor.Yellow);
                Out.WriteLine("[INFO] Listening for connections on port (" + (int.Parse(ConfigurationData.Data["game.tcp.port"]))+ ") ", "", ConsoleColor.Yellow);
                //Out.WriteLine("[INFO Conenction Limit set at (" + (UnixToDateTime.par))
                Out.WriteLine("[INFO] Server Time:" + DateTime.Now + "","", ConsoleColor.Yellow);
        ...
        internal static void PerformShutDown(bool restart)
        {
            ...
            Console.WriteLine(" >> RaidRp Emulator Successfully Shutdown <<");
            IsLive = false;
